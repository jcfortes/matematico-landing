      ...
      <section id="apps" className="py-24 px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {apps.map((app) => {
              const cor = appCorPorStatus(app.status)
              return (
                <div key={app.slug} className={`relative rounded-2xl border ${cor.corBorda} bg-gradient-to-b ${cor.corGrad} p-6 flex flex-col`}>
                  ...
                  {app.tagline && (
                    <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${cor.corTag}`}>{app.tagline}</p>
                  )}
                  ...
                  <ul className="space-y-1.5 mb-6">
                    {app.recursos.map((r) => (
                      <li key={r} className="flex items-center gap-2 text-sm text-white/75">
                        <span className={`text-xs ${cor.corCheck}`}>✓</span>{r}
                      </li>
                    ))}
                    <li className={`flex items-center gap-2 text-sm font-semibold italic pl-5 ${cor.corTag}`}>
                      e muito mais…
                    </li>
                  </ul>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center gap-3 w-full">
            <div className="flex gap-6 text-sm text-white/55 flex-wrap justify-center">
              {apps.map((app) => (
                app.status === 'ativo' && app.url ? (
                  <Link key={app.slug} href={app.url} className="hover:text-emerald-400 transition-colors">{app.nome}</Link>
                ) : (
                  <span key={app.slug} className="text-white/30">{app.nome} (breve)</span>
                )
              ))}
            </div>
